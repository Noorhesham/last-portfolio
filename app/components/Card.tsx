...
const Card = ({
  ...
  children: ReactNode;
  img?: string;
  nohover?: boolean;className?:string
}) => {
  return (
    <div className={`${className||""} min-h-44 border group border-muted-foreground relative p-4 rounded-2xl bg-white w-full overflow-hidden`}>
      {!img && !nohover && (
        <div
      ...
      <div className="relative z-10 flex flex-col items-start gap-2">
        {title && (
          <h2 className="flex items-center   lg:text-xl text-lg gap-2">
            {React.cloneElement(icon, { className: " group-hover:-rotate-6 group-hover:scale-125  duration-150" })}
            {title}
          </h2>
        )}
